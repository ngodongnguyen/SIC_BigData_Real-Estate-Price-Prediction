...
def get_all_regions():
    return sorted([val["name"] for _, val in __dvhc_json_2020.items()])


def get_all_areas(reg_name):
    reg_id = __region_to_id_convert(reg_name, __dvhc_json_2020)
    if pd.isna(reg_id):
        return pd.NA
    return sorted(
        [val["name"] for _, val in __dvhc_json_2020[reg_id]["level2s"].items()]
    )


def get_all_wards(reg_name, are_name):
    reg_id = __region_to_id_convert(reg_name, __dvhc_json_2020)
    are_id = __area_to_id_convert(reg_id, are_name, __dvhc_json_2020)
    if pd.isna([reg_id, are_id]).any():
        return pd.NA
    return sorted(
        val["name"]
        for _, val in __dvhc_json_2020[reg_id]["level2s"][are_id]["level3s"].items()
    )
